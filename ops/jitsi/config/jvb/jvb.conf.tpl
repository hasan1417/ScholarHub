...
{{ $WS_DOMAIN := .Env.JVB_WS_DOMAIN | default $PUBLIC_URL_DOMAIN | trimPrefix "https://" | trimPrefix "http://" | trimSuffix "/" -}}
...
{{ end }}
    ...
    websockets {
        ...
        domain = "{{ $WS_DOMAIN }}"
        tls = {{ if $WS_TLS }}true{{ else }}false{{ end }}
