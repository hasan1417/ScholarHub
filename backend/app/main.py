...
async def health_detailed() -> Dict[str, object]:
    ...
    except Exception as e:  # pragma: no cover - infrastructure health reporting
        resp["db"] = {
            "status": "error",
            "error": str(e),
    ...
    if redis_lib:
        ...
        try:
            ...
            resp["redis"] = {
                ...
                "elapsed_ms": round((time.time() - t1) * 1000.0, 2),
            }
        except Exception as e:  # pragma: no cover - infrastructure health reporting
            resp["redis"] = {
                "status": "error",
...
if settings.PROJECTS_API_ENABLED:
    from app.api.v1 import projects  # local import so flag can disable route  # noqa: WPS433

    app.include_router(projects.router, prefix="/api/v1/projects", tags=["projects"])
if settings.PROJECT_REFERENCE_SUGGESTIONS_ENABLED:
    from app.api.v1 import project_references, project_discovery  # noqa: F401,WPS433
