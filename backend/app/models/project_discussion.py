...
class DiscussionArtifactFormat(str, enum.Enum):
    MARKDOWN = "markdown"
    PDF = "pdf"
    LATEX = "latex"
    TEXT = "text"
...
class DiscussionArtifact(Base):
    """Stores downloadable artifacts generated by the AI assistant."""
    ...
    format = Column(
        Enum(
            ...
            name="discussionartifactformat",
            values_callable=lambda enum_cls: [member.value for member in enum_cls],
        ),
        nullable=False,
        default=DiscussionArtifactFormat.MARKDOWN,
    )
    artifact_type = Column(String(100), nullable=False, default="document")  # summary, literature_review, notes, etc.
    content_base64 = Column(Text, nullable=False)  # Base64 encoded content
    ...
    channel = relationship("ProjectDiscussionChannel", back_populates="artifacts")
    creator = relationship("User")

    def __repr__(self) -> str:
        return f"<DiscussionArtifact(id={self.id}, title={self.title}, format={self.format})>"
